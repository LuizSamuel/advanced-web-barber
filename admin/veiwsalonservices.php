...
require('./phpincludes/veiwsalonservices.inc.php');
...
$query = "SELECT * FROM salonservices ORDER BY $sort LIMIT $offset, $categoriesPerPage";

// Searching
$search = isset($_GET['search']) ? $_GET['search'] : '';
if (!empty($search)) {
    $search = $connection->real_escape_string($search);
    $query = "SELECT * FROM salonservices WHERE name LIKE '%$search%' OR description LIKE '%$search%' ORDER BY $sort LIMIT $offset, $categoriesPerPage";
}
...
<head>
    ...
    <title>View Salon Services</title>
    <!-- Bootstrap CSS -->
                    ...
                    <tbody style="background-color: #B2BEB5; width: 30%;">
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                $image_src = strpos($row["image_path1"], './uploads/') === 0 ? $row["image_path1"] : './admin/' . $row["image_path1"];
                                ...
                                echo '<td>
                                        <a href="./phpincludes/edit_veiwsalonservices.inc.php?id=' . $row['id'] . '" class="btn btn-warning"  style="border-radius: 2px; background-color: blue; display: inline-block; padding: 1.5px 5px; text-decoration: none; color: white;">Edit</a>
                                        <a href="./phpincludes/delete_veiwsalonservices.inc.php?id=' . $row['id'] . '" class="btn btn-danger" style="border-radius: 2px; background-color: red; display: inline-block; padding: 1.5px 5px; text-decoration: none; color: white;" onclick="return confirm(\'Are you sure you want to delete this product?\')">Delete</a>
                                    </td>';
                                echo '</tr>';
                            }
                        } else {
